...
function renderForecast(side,loc,w){$('#'+side+'ForecastName').textContent=loc.name.toUpperCase();const highs=w.daily.temperature_2m_max||[], lows=w.daily.temperature_2m_min||[], rain=w.daily.precipitation_sum||[], codes=w.daily.weather_code||[], dates=w.daily.time||[];const avg=highs.length?highs.reduce((s,n)=>s+n,0)/highs.length:0,wet=rain.reduce((s,n)=>s+(n||0),0);$('#'+side+'WeatherSummary').textContent=`AVG HIGH ${Math.round(avg)}° · ${wet.toFixed(2)} IN PRECIP`;
 $('#'+side+'Forecast').innerHTML=dates.map((d,i)=>{const dt=new Date(d+'T12:00:00');return`<div class="forecast-day"><span>${dt.toLocaleDateString('en-US',{weekday:'short'}).toUpperCase()}</span><i class="${isWet(codes[i])?'rain':''}" title="${esc(wxText(codes[i]))}"></i><b>${Math.round(highs[i])}° / ${Math.round(lows[i])}°</b><small>${esc(wxText(codes[i]))}<br>${(rain[i]||0).toFixed(2)} IN</small></div>`}).join('')}
...
function renderClimate(){const A=state.climate.from,B=state.climate.to;if(!A||!B)return;$('#monthAxis').innerHTML=MONTHS.map(m=>`<span>${m}</span>`).join('');renderClimateSide('fromClimate','from',state.from,A);renderClimateSide('toClimate','to',state.to,B);$('#climateLoading').classList.add('done');const avg=x=>x.reduce((s,m)=>s+(m.hi+m.lo)/2,0)/12,prec=x=>x.reduce((s,m)=>s+m.p,0),tempDiff=avg(B)-avg(A),precDiff=prec(B)-prec(A);let maxI=0,maxD=-1;A.forEach((m,i)=>{const d=Math.abs(B[i].hi-m.hi);if(d>maxD){maxD=d;maxI=i}});$('#climateTakeaways').innerHTML=`<article class="takeaway"><span>AVERAGE TEMPERATURE SHIFT</span><b>${tempDiff>=0?'+':''}${tempDiff.toFixed(1)}°F</b><p>${tempDiff>3?'Expect a noticeably warmer destination climate.':tempDiff<-3?'Expect a noticeably cooler destination climate.':'Overall temperatures are fairly similar.'}</p></article><article class="takeaway"><span>ANNUAL PRECIPITATION SHIFT</span><b>${precDiff>=0?'+':''}${precDiff.toFixed(1)} IN</b><p>${precDiff>5?'The destination has been materially wetter in the recent archive.':precDiff<-5?'The destination has been materially drier in the recent archive.':'Recent annual precipitation is relatively close.'}</p></article><article class="takeaway"><span>BIGGEST HIGH-TEMP CHANGE</span><b>${MONTHS[maxI]} · ${B[maxI].hi-A[maxI].hi>=0?'+':''}${(B[maxI].hi-A[maxI].hi).toFixed(0)}°</b><p>This is the month where recent average daily highs diverge the most.</p></article>`;renderPlan()}
function renderClimateSide(id,side,loc,data){const avgHi=data.reduce((s,x)=>s+x.hi,0)/12,avgLo=data.reduce((s,x)=>s+x.lo,0)/12,prec=data.reduce((s,x)=>s+x.p,0);$('#'+id).className=`climate-side ${side}`;$('#'+id).innerHTML=`<div class="climate-title"><h3>${esc(loc.name)}</h3><span>3-YEAR RECENT BASELINE</span></div>`+data.map((x,i)=>{const left=Math.max(0,Math.min(100,(x.lo+10)/120*100)),right=Math.max(left+3,Math.min(100,(x.hi+10)/120*100));return`<div class="climate-month"><span>${MONTHS[i]}</span><div class="temp-track"><i class="temp-range" style="left:${left}%;width:${right-left}%"></i></div><b>${Math.round(x.lo)}° / ${Math.round(x.hi)}°</b></div>`}).join('')+`<div class="climate-summary"><div><span>AVG HIGH</span><b>${Math.round(avgHi)}°</b></div><div><span>AVG LOW</span><b>${Math.round(avgLo)}°</b></div><div><span>PRECIP / YR</span><b>${prec.toFixed(1)}″</b></div></div>`}

/* budget */
['currentHousing','newHousing','movingQuote','moveMpg','moveGas','hotelNights','hotelRate','otherCost'].forEach(id=>$('#'+id).addEventListener('input',calcBudget));
function n(id){return +$('#'+id).value||0}function calcBudget(){const miles=state.route?.miles||0, fuel=n('moveMpg')?miles/n('moveMpg')*n('moveGas'):0,truck=n('movingQuote'),hotel=n('hotelNights')*n('hotelRate'),other=n('otherCost'),one=truck+fuel+hotel+other,monthly=n('newHousing')-n('currentHousing'),annual=monthly*12;$('#oneTimeCost').textContent=money(one);$('#fuelMoveCost').textContent=`Includes ${money(fuel)} estimated driving fuel`;$('#annualHousingDelta').textContent=`${annual>=0?'+':''}${money(annual)}`;$('#monthlyHousingDelta').textContent=`${monthly>=0?'+':''}${money(monthly)} / month`;const mx=Math.max(truck,fuel,hotel,other,1);[['barTruck',truck],['barFuel',fuel],['barHotel',hotel],['barOther',other]].forEach(([id,x])=>$('#'+id).style.transform=`scaleX(${Math.max(.02,x/mx)})`)}

/* action plan */
['hasVehicle','hasPets','hasKids','isRenter','remoteWork'].forEach(id=>$('#'+id).addEventListener('change',renderPlan));
const PLAN_KEY='movedesk.plan.v1';function planDone(){try{return JSON.parse(localStorage.getItem(PLAN_KEY)||'{}')}catch(e){return{}}}function setDone(k,v){const x=planDone();x[k]=v;localStorage.setItem(PLAN_KEY,JSON.stringify(x))}
function routeKey(){return state.from&&state.to?`${state.from.name}-${state.from.admin1}|${state.to.name}-${state.to.admin1}`:'route'}
...
function renderPlan(){if(!state.from||!state.to)return;const differentState=(state.from.country_code==='US'&&state.to.country_code==='US'&&state.from.admin1&&state.to.admin1&&state.from.admin1!==state.to.admin1),long=(state.route?.miles||0)>500,vehicle=$('#hasVehicle').checked,pets=$('#hasPets').checked,kids=$('#hasKids').checked,renter=$('#isRenter').checked,remote=$('#remoteWork').checked;const tempShift=state.climate.from&&state.climate.to?state.climate.to.reduce((s,x)=>s+x.hi,0)/12-state.climate.from.reduce((s,x)=>s+x.hi,0)/12:0;
 ...
 renderList('beforeList',before);renderList('arrivalList',arrival);renderList('monthList',month);renderOfficialLinks(differentState)}
function renderList(id,items){const done=planDone(),rk=routeKey();$('#'+id).innerHTML=items.map(x=>{const key=`${rk}:${x.id}`,on=!!done[key];return`<label class="check-item ${on?'done':''}" data-key="${esc(key)}"><input type="checkbox" ${on?'checked':''}><div><b>${esc(x.title)}</b><p>${esc(x.copy)}</p></div></label>`}).join('');$$('#'+id+' .check-item').forEach(l=>l.addEventListener('change',()=>{l.classList.toggle('done',l.querySelector('input').checked);setDone(l.dataset.key,l.querySelector('input').checked)}))}
